...
// Approach
// This code is used to find the longest common prefix of an array of strings, which is defined as 
// the longest string that is a prefix of all the strings in the array. 
// By sorting the array and then comparing the first and last elements, 
// the code is able to find the common prefix that would be shared by all strings in the array.

// Sort the elements of an array of strings called "strs" in lexicographic (alphabetical) 
// order using the Arrays.sort(strs) method.
...
// Within the while loop, check if the character at the current index in s1 is equal to the 
// character at the same index in s2. If the characters are equal, increment the value of idx by 1.
...
// This is because most of the common sorting algorithms like quicksort, mergesort, 
...
// The space used by the two string variables s1 and s2 is proportional to the length of the longest 
// string in the array. Therefore, the space complexity is O(1) as it does not depend on 
...
// The longest common prefix of all the strings is "fl", which is located at the beginning of the 
// first string "flight" and the second string "flow". Therefore, by comparing the first and last strings of 
// the sorted array, we can easily find the longest common prefix.
